...
class Pro3_64010516 {
    public static String findRow(int[][] matrix, int target) {
        String list = "";
        boolean isSame;
    ...
    public static String findColumn(int[][] matrix, int target) {
        String list = "";
        boolean isSame;
        for (int i = 0; i < matrix.length; i++) {
            isSame = true;
    ...
    public static void main(String[] args) {
        ...
        printMatrix(matrix);
        // Find Row
        if (findRow(matrix, 0) != "" || findRow(matrix, 1) != "") {
            if (findRow(matrix, 0) != "") {
                System.out.println("All 0s in row " + findRow(matrix, 0));
            }

            if (findRow(matrix, 1) != "") {
                System.out.println("All 1s in row " + findRow(matrix, 1));
            }
        } else {
            System.out.println("No same numbers on a row");
        }
        // Find Column
        if (findColumn(matrix, 0) != "" || findColumn(matrix, 1) != "") {
            if (findColumn(matrix, 0) != "") {
                System.out.println("All 0s in column " + findColumn(matrix, 0));
            }

            if (findColumn(matrix, 1) != "") {
                System.out.println("All 1s in column " + findColumn(matrix, 1));
            }
        } else {
